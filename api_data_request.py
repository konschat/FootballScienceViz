...
headers = {
	"X-RapidAPI-Key": "16b5f87f18mshbfa79f559bb3fd4p150c5fjsnb91911db164b",
...
def get_api_data(base_url, end_url):
    ...
    headers = {'X-RapidAPI-Key': '16b5f87f18mshbfa79f559bb3fd4p150c5fjsnb91911db164b'}
...
base_url = 'https://api-football-v1.p.rapidapi.com/v3/timezone'
...
def req_prem_fixtures_id(season_code, year=YEAR_str):
    ...
    save_api_output(f'{year}_premier_league_fixtures', premier_league_fixtures_sliced,
                    json_data_path='Football_Prediction_Project-master/prem_clean_fixtures_and_dataframes/')

    # loading the json file as a DataFrame
    premier_league_fixtures_df = read_json_as_pd_df(f'{year}_premier_league_fixtures.json',
                                                    json_data_path='Football_Prediction_Project-master/prem_clean_fixtures_and_dataframes/')
...
def load_prem_fixtures_id(year=YEAR_str):
    premier_league_fixtures_df = read_json_as_pd_df(f'{year}_premier_league_fixtures.json',
                                                    json_data_path='prem_clean_fixtures_and_dataframes/')
    return premier_league_fixtures_df
...
fixtures_clean = pd.DataFrame(
    {'Fixture ID': fixtures['fixture_id'], 'Game Date': fixtures['Game Date'], 'Home Team ID': fixtures['HomeTeamID'],
     'Away Team ID': fixtures['AwayTeamID'], 'Home Team Goals': fixtures['goalsHomeTeam'],
     'Away Team Goals': fixtures['goalsAwayTeam'], 'Venue': fixtures['venue'], 'Home Team': fixtures['Home Team'],
     'Away Team': fixtures['Away Team'], 'Home Team Logo': fixtures['Home Team Logo'],
     'Away Team Logo': fixtures['Away Team Logo']})
...
fixtures_clean_2019_2020_2021 = pd.read_csv(
    'prem_clean_fixtures_and_dataframes/2019_2020_2021_premier_league_fixtures_df.csv')
...
fixtures_clean_combined.to_csv('prem_clean_fixtures_and_dataframes/2019_2020_2021_2022_premier_league_fixtures_df.csv',
                               index=False)
